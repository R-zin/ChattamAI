...
## Phase 2 — Plan parsing (OCR) [DONE v1 · layout-aware FUTURE]
- **OCR v1 (done, layout-free):** opt-in `POST /api/check/plan-ocr` endpoint +
  gated image hook in `load_plan_text`, behind `OCR_ENABLED` (see `app/rag/ocr.py`).
  Images and image-only PDFs are OCR'd (Tesseract via PyMuPDF/Pillow), normalised,
  and fed through the existing `Path -> str` seam. Requires the system Tesseract binary.
- **Future (layout-aware):** recover dimensions/setbacks/table structure spatially
  (bounding boxes, table extraction) rather than as raw OCR words; confidence scoring.
