...
4. **Image-only floor plans use opt-in OCR.** `ingestion.py` handles text-based
   PDFs/txt directly; image and image-only-PDF plans are read through the
   opt-in OCR pipeline (`app/rag/ocr.py`) behind `Settings.ocr_enabled` and the
   `POST /api/check/plan-ocr` endpoint (feature-flagged, requires the system
   Tesseract binary). OCR is **layout-free v1** (words, not geometry); true spatial /
   table layout analysis is future work.
...
- **Plan parsing:** OCR is shipped as a layout-free v1 (`POST /api/check/plan-ocr`,
  opt-in via `OCR_ENABLED`). Next: layout/table-aware extraction (recover dimensions,
  setbacks and table structure spatially rather than as raw words).
